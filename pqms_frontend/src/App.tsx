...
export default function App() {
  ...
  // ⭐ NEW: Tracking state
  const [trackingProcessId, setTrackingProcessId] = useState<number | null>(null);
  const [trackingExecutionId, setTrackingExecutionId] = useState<number | null>(null);
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case "process":
        return (
          <ProcessManagement
            onTrackProcess={(processId: number) => {
              setTrackingProcessId(processId);
              setActiveTab("process-tracking");
            }}
          />
        );
      ...
      // ⭐ NEW: Project-level tracking screen
      case "process-tracking":
        ...
        return (
          <ProcessTracking
            processSheetId={trackingProcessId}
            onBack={() => setActiveTab("process")}
            onSelectExecution={(executionId) => {
              setTrackingExecutionId(executionId);
              setActiveTab("execution-tracking");
            }}
          />
        );

      // ⭐ NEW: Execution-level detail tracking
      case "execution-tracking":
        ...
        return (
          <ExecutionTracking
            ...
            onBack={() => setActiveTab("process-tracking")}
          />
        );
        ...
        <div className="flex gap-1 overflow-x-auto">
          {menuItems.map((item) => (
            <Button
              ...
              variant="ghost"
              size="sm"
              className={`flex-shrink-0 ${activeTab === item.id ? "bg-gray-100" : ""}`}
